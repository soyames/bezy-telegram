...
function applyLocale() {
  ...
  setText('discoverable-label', t('app.show_profile')); setText('save-profile', t('app.save_profile')); setText('language-title', t('app.language')); setText('legal-title', t('app.legal_privacy')); setText('privacy-link', t('app.privacy')); setText('terms-link', t('app.terms')); setText('support-title', t('app.support_title')); setText('support-intro', t('app.support_intro')); setText('support-bot-btn', t('app.support_help')); setText('support-history-btn', t('app.support_history')); setText('support-formal', t('app.support_formal')); setText('support-expectation', t('app.support_expectation')); setText('support-email-btn', t('app.support_email'));
...
function openTelegramLink(url) {
  if (!url) return;
  // t.me links and Telegram-internal tg:// deep links (e.g. tg://user?id= for a matched
  // user without a @username) go through Telegram's native opener; the in-app browser
  // fallback is for ordinary https links only, and tg:// has no browser meaning at all.
  if (tg?.openTelegramLink && (url.startsWith('https://t.me/') || url.startsWith('tg://'))) { tg.openTelegramLink(url); return; }
  if (tg?.openLink && url.startsWith('https://')) { tg.openLink(url); return; }
...
// is used; without one, Telegram's numeric-user deep link resolves the chat from the id the
// match card already carries. A match is the one place the numeric id is released, and the
// link only ever leaves the device through Telegram's native opener.
...
function chatButtonHtml(match) {
  const link = chatLinkFor(match);
  if (!link) return '';
  return `<button class="match-open" data-chat="${escapeHtml(link)}" data-match="${escapeHtml(match.matchId || '')}" type="button">${escapeHtml(chatLabel(match))}</button>`;
}
...
function renderMatches() {
  ...
  grid.innerHTML = state.matches.map((match) => {
    ...
    // Primary conversation action first; starters and safety stay secondary behind it.
    const chat = chatButtonHtml(match);
    return `<article class="match-card"><div class="match-photo">${image}</div><div class="match-info"><b>${escapeHtml(match.displayName || t('app.bezy_member'))}${match.age ? `, ${escapeHtml(match.age)}` : ''}</b><span>${escapeHtml(match.city || '')}</span>${whyMatchedHtml(match)}${chat}<button class="match-open" data-starters="${escapeHtml(match.id)}" type="button">${escapeHtml(t('app.starters_title'))}</button><button class="match-open" data-actions="${escapeHtml(match.id)}" type="button">${escapeHtml(t('app.safety_actions'))}</button></div></article>`;
  }).join('');
  ...
  conversations.innerHTML = state.matches.map((match) => {
    ...
    const action = chatButtonHtml(match);
