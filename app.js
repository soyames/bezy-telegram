...
function languageFromTelegram() {
  const code = state.telegramUser?.language_code || tg?.initDataUnsafe?.user?.language_code || navigator.language || 'en';
...
function applyLocale() {
  setText('tagline', t('app.tagline'));
  setText('nav-discover', t('app.discover'));
  setText('nav-matches', t('app.matches'));
  setText('nav-messages', t('app.messages'));
  setText('nav-profile', t('app.profile'));
  setText('discover-title', t('app.discover_title'));
  setText('discover-intro', t('app.discover_intro'));
  setText('privacy-pill', t('app.private_by_design'));
  setText('for-you', t('app.for_you'));
  setText('filterBtn', t('app.filters'));
  setText('premium-copy', t('app.premium_copy'));
  setText('premiumBtn', t('app.view_membership'));
  setText('your-matches', t('app.your_matches'));
  setText('discoverBtn', t('app.discover'));
  setText('matches-premium-copy', t('app.matches_premium_copy'));
  document.querySelectorAll('.premium-action').forEach((node) => { node.textContent = t('app.unlock_premium'); });
  setText('messages-title', t('app.messages'));
  setText('protected-label', t('app.protected_by_bezy'));
  setText('profile-title', t('app.my_profile'));
  setText('profile-refresh', t('app.refresh'));
  setText('edit-profile', t('app.edit_profile'));
  setText('display-name-label', t('app.display_name'));
  setText('age-label', t('app.age'));
  setText('city-label', t('app.city'));
  setText('gender-label', t('app.gender'));
  setText('seeking-label', t('app.seeking'));
  setText('interests-label', t('app.interests'));
  setText('bio-label', t('app.bio'));
  setText('interests', t('app.interests_placeholder'));
  setText('discoverable-label', t('app.show_profile'));
  setText('save-profile', t('app.save_profile'));
  setText('language-title', t('app.language'));
  setText('legal-title', t('app.legal_privacy'));
  setText('privacy-link', t('app.privacy'));
  setText('terms-link', t('app.terms'));
  setText('people-count', '—');
  setText('match-percent', '—');
  setText('new-count', '—');
  setText('message-empty', t('app.no_conversations'));
  setText('matches-empty', t('app.no_matches'));
...
async function api(path, options = {}) {
  ...
  if (!response.ok) throw new Error(data.error || t('app.error_generic'));
  return data;
}
...
function showView(view, options = {}) {
  ...
  state.view = target;
  document.querySelectorAll('.view').forEach((node) => node.classList.toggle('active', node.id === `${target}-view`));
  document.querySelectorAll('.nav button').forEach((button) => button.classList.toggle('active', button.dataset.view === target));
  if (target === 'discover') loadDiscover();
  if (target === 'matches') loadMatches();
  if (target === 'messages') loadMatches(true);
  if (target === 'profile') renderAccount();
...
function renderAccount() {
  ...
  $('profile-status').textContent = account.profileComplete ? t('app.show_profile') : t('app.complete_profile');
  $('display-name').value = profile.displayName || account.firstName || '';
  $('age').value = profile.age || '';
  $('city').value = profile.city || '';
  $('gender').value = profile.gender || 'prefer_not_to_say';
  $('seeking').value = profile.seeking || 'everyone';
...
function renderDiscover() {
  ...
  $('passBtn').onclick = () => actOnCurrent('pass');
  $('superBtn').onclick = () => actOnCurrent('super');
  $('likeBtn').onclick = () => actOnCurrent('like');
  setText('people-count', String(state.profiles.length));
  setText('match-percent', state.profiles.length ? '—' : '0%');
  setText('new-count', '—');
}

async function loadDiscover() {
  if (!state.account?.profileComplete) {
    state.profiles = [];
    renderDiscover();
    return;
  }
  ...
  try {
    ...
    $('discover-content').innerHTML = `<div class="empty">${escapeHtml(error.message || t('app.error_generic'))}</div>`;
  } finally {
...
function renderMatches() {
  ...
  if (!state.matches.length) {
    grid.innerHTML = '';
    empty.textContent = t('app.no_matches');
    empty.classList.remove('hidden');
    $('conversation-list').innerHTML = '';
    $('message-empty').classList.remove('hidden');
    return;
  }
  empty.classList.add('hidden');
  grid.innerHTML = state.matches.map((match) => {
    const initial = (match.displayName || 'B').charAt(0).toUpperCase();
    const image = match.photoUrl ? `<img src="${escapeHtml(match.photoUrl)}" alt="">` : escapeHtml(initial);
    const button = match.username
      ? `<button class="match-open" data-chat="${escapeHtml(match.username)}" type="button">${escapeHtml(t('app.open_chat'))}</button>`
      : `<button class="match-open" data-nochat="1" type="button">${escapeHtml(t('app.open_chat'))}</button>`;
  ...
  $('conversation-list').innerHTML = state.matches.map((match) => {
    const initial = (match.displayName || 'B').charAt(0).toUpperCase();
  ...
  $('message-empty').classList.add('hidden');
  $('conversation-list').querySelectorAll('[data-chat]').forEach((button) => button.onclick = () => openTelegramLink(`https://t.me/${button.dataset.chat}`));
}
...
async function loadMatches(messagesView = false) {
  try {
    ...
    if (messagesView) showToast(error.message || t('app.error_generic'));
  }
}
...
async function saveProfile(event) {
  ...
  try {
    const data = await api(API.profile, { body: { profile } });
    state.account = data.profile;
    renderAccount();
    showToast(t('app.profile_saved'));
    if (state.account.profileComplete && state.account.discoverable) showView('discover');
  } catch (error) {
    showToast(error.message || t('app.error_generic'));
  }
}
...
async function loadAccount() {
  ...
  state.account = data.profile;
  state.telegramUser = state.telegramUser || data.profile;
  renderAccount();
  return data;
}
...
function bindEvents() {
  ...
  $('settingsBtn').onclick = () => showView('profile');
  $('discoverBtn').onclick = () => showView('discover');
  $('premiumBtn').onclick = () => showView('premium');
  document.querySelectorAll('.premium-action').forEach((button) => button.onclick = () => showView('premium'));
  $('profile-form').addEventListener('submit', saveProfile);
  $('profile-refresh').onclick = async () => { try { await loadAccount(); showToast(t('app.profile_saved')); } catch (error) { showToast(error.message || t('app.error_generic')); } };
  $('filterBtn').onclick = () => showToast(t('app.filters'));
  document.querySelectorAll('[data-language]').forEach((button) => button.addEventListener('click', async () => {
    try { await loadLocale(button.dataset.language); renderAccount(); } catch { showToast(t('app.error_generic')); }
  }));
}

async function init() {
  if (!tg?.initData) {
    document.body.innerHTML = `<main style="padding:40px;font-family:system-ui;text-align:center"><h2>Bezy</h2><p>${escapeHtml('Open Bezy from Telegram to continue.')}</p></main>`;
    return;
  }
  ...
  const storedLanguage = localStorage.getItem('bezy-language');
