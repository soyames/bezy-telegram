...
function renderDiscover() {
  ...
  if (!profile) {
    ...
    if (state.emptyReason === 'eligibility') {
      host.innerHTML = `<div class="empty"><p>${escapeHtml(t('app.empty_eligibility'))}</p><button class="ghost-btn" id="empty-check-later" type="button">${escapeHtml(t('app.check_later'))}</button></div>`;
      $('empty-check-later').onclick = async () => { try { await loadDiscover(); } catch { /* error state already rendered */ } };
      return;
    }
