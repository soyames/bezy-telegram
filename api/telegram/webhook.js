...
async function sendCommand(chatId, command, language) {
  ...
  if (command === 'premium') {
    await telegramApi('sendMessage', { chat_id: chatId, text: language === 'fr' ? '✨ Bezy Premium arrive bientôt. Les achats seront intégrés à Telegram Stars.' : '✨ Bezy Premium is coming soon. Purchases will be integrated with Telegram Stars.', reply_markup: { inline_keyboard: [[uiButton(language, 'premium')]] } });
    return;
  }
